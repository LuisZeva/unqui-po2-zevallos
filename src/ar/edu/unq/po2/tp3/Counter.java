...
public class Counter {
    ...
    public int maxMultiploDeXYHastaMil(int numero1, int numero2) {
    	int numeroActual = 1000;
    	while ((!esMultiploDeXY(numeroActual, numero1, numero2)) && (numeroActual > 0)) {
    		numeroActual = numeroActual - 1;
    	}
	
    	return numeroOMenosUno(numeroActual);
    }
    
    private boolean esMultiploDeXY(int numeroAEvaluar, int numero1, int numero2) {
    	return	(esMultiploDe(numeroAEvaluar, numero1) && esMultiploDe(numeroAEvaluar, numero2));
    }
    
    private boolean esMultiploDe(int numeroAEvaluar, int divisor) {
    	return numeroAEvaluar % divisor == 0;
    }
    
    private int numeroOMenosUno(int numero) {
        if (esCero(numero)) {
            return -1;
        } else {
            return numero;
